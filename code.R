...
# removing missing values in ENG (English level) and WAGP (Wages)
dc1 <- dc %>%
        select(var1$variables) %>%
        filter(!is.na(ENG), !is.na(WAGP)) 
...
SCHL_d <- data.frame(SCHL = as.character(1:24), 
...
# function for converting income variables (integer) to categorical variables
# by quantile
divide_quantile <- function(vec) {
        q <- quantile(vec)
        assigned <- c()
        for (x in vec) {
                if (x > q[4]) {
                        assigned <- c(assigned, "High") 
                } else if (x > q[3]) {
                        assigned <- c(assigned, "Upper Middle")
                } else if (x > q[2]) {
                        assigned <- c(assigned, "Lower Middle")
                } else {
                        assigned <- c(assigned, "Low")
                }
        }
        return(assigned)
}
...
dc3 <- dc2 %>%
        ...
        # converting WAGP(Wage for past 12 months) and PINCP (total income) to
        # categorical variables
        mutate(Salary = divide_quantile(WAGP),
               Total_Income = divide_quantile(PINCP)) 
...
english_table1 <- as.data.frame(prop.table(table(dc3$English, 
                                                 dc3$Work_Insurance), 
                                           margin = 1))
english_table2 <- as.data.frame(prop.table(table(dc3$English, 
                                                 dc3$Health_Insurance_Covered), 
                                           margin = 1)) 
...
# cp plotting function
cp <- function(Input_categorial) {
        m <- rpart(Input_categorial ~ 
                           English + 
                           Education,
                   data = dc4, method = "class")
        return(plotcp(m))
...
# cp_work = 0.053
# cp of others = NaN
cp_work <- cp(dc4$Work_Insurance)
cp_private <- cp(dc4$Private_Insurance)
cp_insurance <- cp(dc4$Health_Insurance_Covered)
...
# Predict coverage of work insurance via classification tree (+ pruning)
work_tree_model <- rpart(Work_Insurance ~ 
                                 English + 
                                 Education,
                         data = dc4, 
                         method = "class")
work_pruned <- prune(work_tree_model, cp = 0.053)
work_pruned_tree_prediction <- predict(work_pruned, 
                                       newdata = dc4, 
                                       type = "class")
work_tree_accuracy_pruned <- mean(work_pruned_tree_prediction == dc4$Work_Insurance)
...
pred_table_vis <- formattable(pred_table, 
                              list(area(col = 2:4) ~ color_tile("white", "lightpink")))
